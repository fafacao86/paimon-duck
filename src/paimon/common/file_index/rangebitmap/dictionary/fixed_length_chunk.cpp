...
Result<bool> FixedLengthChunk::TryAdd(const Literal& key) {
    ...
    PAIMON_RETURN_NOT_OK(serializer_.value()(keys_stream_out_.value(), key));
...
Result<Literal> FixedLengthChunk::GetKey(const int32_t index) {
    ...
    if (!keys_.has_value()) {
        ...
        PAIMON_RETURN_NOT_OK(input_stream_->Read(keys_->get()->data(), keys_length_));
    ...
    PAIMON_RETURN_NOT_OK(keys_stream_in_.value()->Seek(index * fixed_length_));
    return deserializer_.value()(keys_stream_in_.value(), pool_.get());
}
...
Result<PAIMON_UNIQUE_PTR<Bytes>> FixedLengthChunk::SerializeChunk() const {
    ...
    PAIMON_RETURN_NOT_OK(serializer_.value()(data_out, key_));
    data_out->WriteValue<int32_t>(code_);
    data_out->WriteValue<int32_t>(offset_);
    data_out->WriteValue<int32_t>(size_);
    data_out->WriteValue<int32_t>(static_cast<int32_t>(keys_stream_out_.value()->CurrentSize()));
...
Result<PAIMON_UNIQUE_PTR<Bytes>> FixedLengthChunk::SerializeKeys() const {
    return MemorySegmentUtils::CopyToBytes(keys_stream_out_.value()->Segments(), 0,
                                           static_cast<int32_t>(keys_stream_out_.value()->CurrentSize()), pool_.get());
}
