...
Result<int32_t> ChunkedDictionary::Find(const Literal& key) {
    ...
    if (low == 0) {
        return -(low + 1);  // This make sure the return value is negative if not found
    }
...
/// - If current chunk is full or the size limit cannot fit a single key, flushes it and remove the
/// chunk
///
